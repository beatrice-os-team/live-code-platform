var this_html = {
    ...
    async editor_run() {
        try {
            let load = layui.layer.msg('语法分析进行中 ... ', {
                icon: 16,
                shade: 0.01
            });
            ...
            if (!inputCode.trim()) {
                document.getElementById('editor-result').innerHTML = 
                    `<span style="color:orange">请输入一些代码进行语法分析</span><br>`;
            ...
            // 使用语法分析演示模块
            let losuModule = window.LosuParserModule;
            if (!losuModule) {
                // 尝试加载语法分析WASM模块
                if (typeof LosuParser === 'undefined') {
                    document.getElementById('editor-result').innerHTML += 
                ...
                // 缓存模块以供后续使用
                window.LosuParserModule = losuModule;
            }
            
            // 直接调用导出的函数
            if (losuModule && losuModule._parser_demo) {
                try {
                    // 手动分配内存并转换字符串
                    ...
                    // 调用语法分析演示函数
                    losuModule._parser_demo(strPtr);
                    
                    // 释放内存
                    losuModule._free(strPtr);
                } catch (parserError) {
                    console.warn('parser_demo函数调用失败，使用通用运行函数:', parserError);
                    try {
                        // 回退到通用运行函数
                        const strLen = losuModule.lengthBytesUTF8(inputCode) + 1;
                        const strPtr = losuModule._malloc(strLen);
            ...
            console.error('语法分析出错:', e);
            document.getElementById('editor-result').innerHTML += 
                `<span style="color:red">语法分析出错: ${e.message}</span><br>`;
            layui.layer.close(load);
        }
    },
    async loadscript(url) {
        try {
            let res = await fetch('/assets/script/parser/' + url);
            if (!res.ok) {
...
layui.util.on('lay-on', {
    iframe: async function () {
        // iframe 层
        try {
            let res = await fetch("/assets/markdown/parser.md");
            let text = "";
            if (res.status == 200) {
                text = await res.text();
            } else {
